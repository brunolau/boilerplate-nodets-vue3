...
(function ($) {
    ...
    function Plot(placeholder, data_, options_, plugins) {
        ...
        function insertLegend() {
            ...
            //Inviton hack, add summary row
            if (entries.length > 0) {
                if (options.legend.showSummary != null && options.legend.showSummary == true) {
                    var sum = 0;
                    for (var i = 0; i < entries.length; ++i) {
                        try {
                            sum += entries[i].data[0][1];
                        } catch (e) {}
                    }

                    var summaryLabel = options.legend.summaryLabel || " Total [{0}]";
                    summaryLabel = summaryLabel.replace("{0}", sum);

                    fragments.push(
                        '<td class="legendColorBox"><div style="border:1px solid ' +
                            options.legend.labelBoxBorderColor +
                            ';padding:1px"><div style="width:4px;height:0;border:5px solid white;overflow:hidden"></div></div></td>' +
