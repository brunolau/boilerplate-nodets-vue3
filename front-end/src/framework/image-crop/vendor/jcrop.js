...
(function ($) {
    ...
    $.extend(Jcrop, {
        ...
        canvasClone: function (imgel) {
            ...
            $(canvas).width(imgel.width).height(imgel.height), (canvas.width = imgel.naturalWidth);
        ...
        propagate: function (plist, config, obj) {
            for (var i = 0, l = plist.length; i < l; i++) if (config.hasOwnProperty(plist[i])) obj[plist[i]] = config[plist[i]];
        },
        // }}}
    ...
    $.extend(CanvasStage.prototype, {
        ...
        redraw: function () {
            ...
            this.context.save();
            this.clear();
            var imgWidth = this.imgsrc.naturalWidth || this.imgsrc.width;
            var imgHeight = this.imgsrc.naturalHeight || this.imgsrc.height;

            if (this.angle == 90 || this.angle == 270) {
                this.context.translate(imgHeight / 2, imgWidth / 2);
            } else {
                this.context.translate(imgWidth / 2, imgHeight / 2);
            }
            ...
            this.context.drawImage(this.imgsrc, -imgWidth / 2, -imgHeight / 2);
    ...
    $.extend(ShadeFilter.prototype, {
        ...
        init: function () {
            ...
            if (!t.attached) {
                ...
                t.container = $("<div />").addClass(t.core.opt.css_shades).prependTo(this.core.container).hide();
            ...
            t.getElement().animate(
                ...
                },
                {
                    ...
                    progress: function (anim) {
                        ...
                        t.stage.setAngle(props.width).setScale(props.height).setOffset(props.top, props.left).redraw();
                    },
                },
            );
        },
    ...
    DragState.prototype = {
        ...
        initEvents: function (e) {
            $(this.eventTarget).on("mousemove.jcrop", this.createDragHandler()).on("mouseup.jcrop", this.createStopHandler());
        },
        // }}}
    ...
    $.extend(ImageLoader, {
        ...
        prototype: {
            ...
            fireCallback: function () {
                ...
                if (typeof this.callback == "function") this.callback.apply(this, this.getDimensions());
            },
            isLoaded: function () {
    ...
    $.extend(JcropTouch, {
        ...
        prototype: {
            ...
            shimDragState: function () {
                ...
                $.Jcrop.component.DragState.prototype.initEvents = function (e) {
                    ...
                    if (e.type.substr(0, 5) == "touch") {
                        $(this.eventTarget).on("touchmove.jcrop.jcrop-touch", t.dragWrap(this.createDragHandler())).on("touchend.jcrop.jcrop-touch", this.createStopHandler());
                    }
            ...
            initEvents: function () {
                ...
                c.container.on("touchstart.jcrop.jcrop-touch", "." + c.opt.css_drag, t.dragWrap(c.startDrag()));
            },
            // }}}
        },
    });
    ...
    $.extend(Selection, {
        ...
        prototype: {
            ...
            callFilterFunction: function (f, args) {
                for (var i = 0; i < this.filter.length; i++) if (this.filter[i][f]) this.filter[i][f](this);
                return this;
            },
            // }}}
            ...
            runFilters: function (b, ord) {
                for (var i = 0; i < this.filter.length; i++) b = this.filter[i].filter(b, ord, this);
                return b;
            },
            // }}}
            //endDrag: function(){{{
            ...
            startDrag: function (e, ord) {
                ...
                if (ord == "move" && t.element.hasClass(t.core.opt.css_nodrag)) return false;
            ...
            insertElements: function () {
                ...
                for (i = 0; i < d.length; i++) fr.append(t.createElement(o.css_dragbars, d[i]));

                for (i = 0; i < h.length; i++) fr.append(t.createElement(o.css_handles, h[i]));

                for (i = 0; i < b.length; i++) fr.append(t.createElement(o.css_borders, b[i]));
            },
            //}}}
        },
    });
    ...
    $.extend(StageDrag.prototype, {
        ...
        // end: function(x,y){{{
            ...
            if (b.w < this.minsize[0] || b.h < this.minsize[1]) this.core.requestDelete();
            else this.sel.focus();
        },
        // }}}
    });
    ...
    $.extend(StageManager.prototype, {
        ...
        tellConfigUpdate: function (options) {
            for (var i = 0, m = this.ui.multi, l = m.length; i < l; i++) if (m[i].setOptions && (m[i].linked || (this.core.opt.linkCurrent && m[i] == this.ui.selection))) m[i].setOptions(options);
        },
        // }}}
        ...
        shimLegacyHandlers: function (options) {
            ...
            $.each(core.opt.legacyHandlers, function (k, i) {
                if (k in options) {
                    tmp = options[k];
                    core.container.off(".jcrop-" + k).on(i + ".jcrop.jcrop-" + k, function (e, s, c) {
                        tmp.call(core, c);
                    });
                    delete options[k];
                }
    ...
    $.extend(Thumbnailer, {
        ...
        prototype: {
            ...
            insertElements: function () {
                ...
                this.element = $("<div />").addClass("jcrop-thumb").width(this.width).height(this.height).append(this.preview).appendTo(this.core.container);
            },
            resize: function (w, h) {
                this.width = w;
                this.height = h;
    ...
    DialDrag.prototype = {
        ...
        mousedown: function () {
            ...
            return function (e) {
                ...
                $(window).on("mousemove.dialdrag", mouseMove).on("mouseup.dialdrag", mouseUp);
    ...
    $.extend(Jcrop.prototype, {
        ...
        getDefaultFilters: function () {
            ...
            for (var i = 0, f = this.opt.applyFilters, l = f.length; i < l; i++) if (this.filter.hasOwnProperty(f[i])) rv.push(this.filter[f[i]]);
        ...
        hasSelection: function (sel) {
            for (var i = 0; i < this.ui.multi; i++) if (sel === this.ui.multi[i]) return true;
        },
        // }}}
        ...
        //addFilter: function(filter){{{
        addFilter: function (filter) {
            for (var i = 0, m = this.ui.multi, l = m.length; i < l; i++) m[i].addFilter(filter);
        ...
        removeFilter: function (filter) {
            for (var i = 0, m = this.ui.multi, l = m.length; i < l; i++) m[i].removeFilter(filter);
        ...
        // refresh: function(){{{
        refresh: function () {
            for (var i = 0, s = this.ui.multi, l = s.length; i < l; i++) s[i].refresh();
        },
        // }}}
        // blurAll: function(){{{
        blurAll: function () {
